...
class Register:
   ...
   def registerDictionary(self, path, format, encoding="utf-8"):
      ...
      # There are formats which don't use indexation
      if format != "Dict":
         self.makeHashTable(path, file)

      wxGetApp().window.SetStatusText(_("Registration complete"))
