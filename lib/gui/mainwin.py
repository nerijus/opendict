...
class MainWindow(wxFrame):
   ...
   def __init__(self, parent, id, title, pos=wxDefaultPosition,
      ...
      menuFile.AppendMenu(105, _("Open from file"), menuFileOpen)
      #menuFile.AppendSeparator()
      menuFile.Append(123, _("Connect to DICT server..."),
                          _("Open connection to DICT server"))
      menuFile.AppendSeparator()
      menuFile.Append(2004, _("Print translation"), "")
      menuFile.Append(2006, _("Page preview"), "")
      menuFile.AppendSeparator()
      ...
      for name in keys:
         ...
         # events.
         #if wx.__version__ != "2.4.1.2":
         #   item.SetBitmap(wxBitmap(os.path.join(home, "pixmaps", "register.xpm"),
         #                           wxBITMAP_TYPE_XPM))
         self.menuDict.AppendItem(item)
      ...
      self.menuDict.AppendMenu(114, _("Add new"), menuDictAdd)

      menuBar.Append(self.menuDict, _("&Dictionaries"))

      menuTools = wxMenu()
      menuTools.Append(122, _("Groups...\tCtrl-G"),
      ...
      EVT_MENU(self, 112, self.onAddFromFile)
   ...
   def onTimerSearch(self, event):
      if self.search != None and self.search.isDone():
         ...
         if type(result[0]) != type(u''):
            self.htmlWin.SetPage(result[0])
            self.history.add(result[0])
         else:
            try:
               ...
               print "Failed to encodode '%s'... to %s, iso-8859-1 used" \
                     % (result[0][:10], self.encoding)
               misc.printError()
               self.htmlWin.SetPage(result[0].encode("iso-8859-1", "replace"))
               self.history.add(result[0].encode("iso-8859-1", "replace"))
   ...
   # TODO: unused until html area is selectable
   # What should we use? Styled Text is too difficult for
   # simple html generation, wxMozilla is not distributed
   # with wxWindows.
